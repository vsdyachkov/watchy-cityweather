...
void drawOutlinedText(Adafruit_GFX &d, int16_t x, int16_t y, const String &text, const GFXfont *font, int textColor) {
  d.setFont(font);

  int borderColor = textColor == GxEPD_BLACK ? GxEPD_WHITE : GxEPD_BLACK;
  d.setTextColor(borderColor);  // Белая обводка
  // Рисуем текст вокруг центра (все соседние пиксели)
  ...
  // Рисуем основной чёрный текст
  d.setTextColor(textColor);
  d.setCursor(x, y);
  d.print(text);
}

void drawLine(Adafruit_GFX &d, int x0, int y0, int x1, int y1, uint16_t c = 1, int dithering = 2) {
